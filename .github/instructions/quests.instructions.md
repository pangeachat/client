...
**Each course keeps its own progress, even when two courses come from the same quest.** A quest is a reusable plan, so a learner can join two courses built from the same one — for instance a focused version and a version with extra side-quest activities. Stars earned in one course count toward that course only; the other course shows its own progress, never a mix of the two ([client#8087](https://github.com/pangeachat/client/issues/8087)). On the world map, whose only question is where to go next, two such courses point at the same next Mission and so read as one destination rather than being counted twice.
