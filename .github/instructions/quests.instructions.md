...
The participant card is roughly one avatar wide and has no room for a label, so the two are distinguished by **form**: course progress always displays as a fraction over a bar, a member's total always as a bare count with no denominator. The full sense is carried in the card's accessible name and its hover tooltip, which cost no space. A denominator added to the participant card would collapse the distinction and should not be — that is the confusion this rule exists to prevent.
