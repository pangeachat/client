...
**Blocking detection and its limit (#8603).** Tile-load failures leave a rate-limited Sentry breadcrumb (`TileLayer.errorTileCallback`), and non-2xx responses are treated as hard errors rather than optimistically decoded — a provider that starts refusing us with an error status is visible in telemetry and degrades on screen to the themed map background instead of flashing. What no cheap check can catch is a provider serving *wrong* tiles with HTTP 200 — exactly #8585's watermark mode. Detecting that would mean pixel-inspecting tiles against a reference, so recurrence of that class is caught only by human eyes on the map; do not read the telemetry as covering it.
