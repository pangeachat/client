...
The boosts sum to at most `0.9`, strictly under one band step (`1`), so they only **reorder within a band** — a joined-course item always outranks a level-appropriate one. That preserves the strict priority above while giving one comparable number to fill slots with and to feed a future model. The headroom up to (but not reaching) one band step is held for a **social-proof / urgency** term (a nearly-full session) once that signal exists; that term must stay under `0.1` so the boosts still sum to strictly less than a band step, otherwise a maxed-out lower-band item could tie one a band above and break the within-band-only guarantee.
