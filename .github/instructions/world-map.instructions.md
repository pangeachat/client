...
The map holds far more than it should ever show at full weight, so every item renders at one of three visual weights ([`PinTier`](../../lib/routes/world/world_map_ranking.dart)) and only a few earn the heavy ones. The counts are an **attention budget** that scales with the visible map size; fewer is always better than crowding. The visual treatment is the [pin-tier frame](https://www.figma.com/design/n2qX4WsnVhYqT2KV6pMVbl/Everything-outside-of-Chat?node-id=12912-348899).
...
| **Large card** | up to 3 (fewer when they can't fit) | [`WorldMapLargeCard`](../../lib/routes/world/world_map_large_card.dart) — the featured callout, simply the top of the score ([Priority matrix](#priority-matrix) below), no separate eligibility: a full preview card shown right on the map — image, title, type, level match, and a **row of stars** for the learner's progress. A live **joinable** session almost always wins it (joinable is the heaviest score term), and shows who is already in the session, its open slots, and a join affordance; when nothing is live, the learner's next-Mission frontier activity earns the slot instead. **Auto-featured** only where there is horizontal room (desktop / column mode); but the same card is the on-demand detail any pin expands to on tap — a **selected** pin (see [Interaction](#interaction) below) — so a selected card renders on a narrow screen too |

A pin's **state** ([`ActivityPinState`](../../lib/routes/world/world_map_ranking.dart)) is carried by color at every tier — the component is `Activity pin v3` in the frame above. There is no locked state: every activity is always playable, so progression only ever *ranks* content, never gates it (the [Priority matrix](#priority-matrix) and [quests.instructions.md](quests.instructions.md)). Two colors remain:
...
**Progress is a fill, not a state.** A learner's progress in an activity is the stars earned toward its total (a star is one awarded activity goal — [activities.instructions.md](activities.instructions.md)); star totals are read from Matrix room state by [`QuestsClientExtension`](../../lib/features/quests/quests_client_extension.dart). It renders by tier: on the **small and mid pins** as an **inner yellow dot whose radius grows with that fraction**, and on the **large card** as a **row of stars**. Empty means untouched, full means finished. Progress is orthogonal to the color state: it never recolors the pin and never hides it, so a finished activity stays on the map as a normal pin — the learner sees their trail without it crowding out the next thing to do. This replaces the earlier discrete "completed" state (a separate gold pin).
...
**What ships.** Both colors plus the progress fill are derived from Matrix room state the client already holds. Progression itself is now purely a ranking input: the client resolves each quest's **next Mission** via [`QuestProgressionResolver`](../../lib/features/quests/quest_progression_resolver.dart) (see [quests.instructions.md](quests.instructions.md)) and the Priority matrix preferences activities in it — no pin is ever grayed out or blocked. A ping leaves no persistent room state, so **pinged** is detected best-effort by scanning recent course-space messages for the host's ping — a proxy whose efficacy we watch before investing in a persistent ping signal.

### Featured, selected, and focused

Beyond its color state and progress fill, a pin can carry one of three emphasis states, managed by [`WorldMapPinsManager`](../../lib/routes/world/world_map_pins_manager.dart). They are **decoupled** — different mechanisms, different lifetimes:
...
**Grouping** de-overlaps the map but is subordinate to the tiers: tier fill runs first, so any item that earns a **mid or large** slot renders at that weight even inside a dense area — it breaks out of the cluster rather than hiding in it. Only the leftover **small** dots that would overlap collapse into a count bubble ([`WorldMapClusterBubble`](../../lib/routes/world/world_map_cluster_bubble.dart)) — the `Grouped` variant, itself state-colored — which expands on zoom or tap. So a cluster only ever hides long-tail items; a high-ranked activity is always visible at its earned weight, never swallowed by the bubble it sits within. The bubble **renders statically, with no entry animation** (only individual pins animate in/out): the clustering layer recreates every bubble's widget state whenever its marker set changes, and that set churns continuously as the viewport pins reload and re-rank on pan/zoom, so a scale-in there reads as constant "re-popping." A count change updates in place.

The reverse of that also holds. Because a large card occupies a box, it **reserves that footprint**: the long-tail de-overlaps *around* it (no count bubble ever forms beneath a card), two large cards never claim overlapping screen space (the lower-scored yields to a mid pin), and a card with no room at a screen edge yields to its dot rather than spilling off. Only **Selected** and **Focused** cards hold their large form against an edge — they nudge the camera instead. The mechanism is step 4 of the [Priority matrix](#priority-matrix) pipeline.
...
**The data the band needs** is nearly all already on the pin: its **level** (CEFR) and **L2** travel today, and activities already link their learning-objectives in the CMS. The choreographer's bbox card already carries those Mission refs (`learning_objective_refs` on each card from `/v2/activities/bbox` via [`ActivityMapRepo`](../../lib/features/quests/repo/activity_map_repo.dart)), so the only remaining gap is client-side: the client must parse them off the card and feed them into the band. The learner's in-scope quests and per-Mission star rollup come from [`QuestProgressionResolver`](../../lib/features/quests/quest_progression_resolver.dart), cached via [`JoinedObjectiveCache`](../../lib/routes/world/joined_objective_cache.dart) and refreshed on course join or leave rather than per re-rank. No new content model, endpoint, or CMS field is required.

**The bbox is the one always-on filter, and tiers are assigned over its survivors.** The current map viewport is a constant member of the [filter set](#filters) — never removable, continuously changing as the learner pans and zooms — and the tier budgets (large up to 3, mid ~5–10) are filled from the in-bbox candidate set against those counts — and the large budget is **further bounded by what physically fits on screen** (step 4 below), so it is a ceiling, not a guarantee. So featuring is emergent from zoom with no change to any item's score: zoomed far out, many candidates compete for the same few slots and a low-score activity renders as a small dot (or, past the working-set cap, not at all — see [Scale boundary](#scale-boundary)); zoomed in, the field thins and that same activity naturally earns a mid or large slot. The bbox only decides membership; the single score then ranks the survivors.
...
3. **Fill, with diversity** — keep one score-ordered candidate list capped at the combined large + mid budget, avoiding a featured set that is all one course, Mission, or cluster. When more items tie for large than its budget (the common case: several joinable sessions live at once), the large slots **rotate through them, about every 5 seconds**, so each gets airtime; with three or fewer they stay static.
4. **Place** — a large card is a *box*, not a point, so that ordered candidate list is laid onto real screen positions last (mid and small ride the cluster layer as before; this step is large-only). Walking the large candidates top-down, a card earns its slot only if its footprint fits the **unclaimed visible safe area**: on-screen — not off an edge, not under an open panel (the `leftOverlayWidth` / `rightOverlayWidth` tracked by [`WorldMapController`](../../lib/routes/world/world_map.dart)) — and not overlapping a card already placed this pass. Footprints are reserved in priority order **Focused → Selected → Featured**, so a deliberately surfaced card keeps its space and the auto-featured set yields around it. A **Featured** candidate that cannot fit yields its slot to the next one down-score that can, and renders as its dot; the large count is therefore **emergent — `min(3, what fits)`**, never a fixed three (a clean single card beats three crammed ones). Long-tail dots whose point falls inside a placed card's footprint drop out of the cluster layer, so a count bubble never forms beneath a card. **Selected** and **Focused** never shrink: when their card would fall off an edge, the camera nudges to bring it fully on-screen instead of yielding.
