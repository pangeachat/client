...
- **On web, only page text reliably switches voice.** VoiceOver on web switches voice for text written into the page. It reads a name given as an `aria-label` in the UI voice whatever its language, and Flutter's web engine names any node that has children with an `aria-label`. Buttons are unreliable: with the same markup, word buttons were read in the target language in some places and in the UI voice in others. So a message or voice-message transcript whose words are their own buttons, and an example chip, which is one button named by its message, also carry their whole text as a text-only node, read first; without buttons, the text is already page text. Other buttons named in the target language (the word card's title in a chat, vocab chips, choice cards, the tutorial greeting) stay one button each and keep their language for iOS and Android. On web they may be read in the UI voice.
...
- **An example chip is its message as text, then one button named by it.** The message's word buttons are left out of the chip, so it is one control with one name. The message is read twice: first as text, which is reliably in its own voice on web, then as the button's name, which has to contain the visible text (WCAG 2.5.3). Tapping the chip opens the toolbar, where the words are.
