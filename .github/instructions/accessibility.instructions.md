...
- **An example chip is one button named by its message.** The message's word buttons are left out of it, so the chip is one control with one name. Tapping it opens the toolbar, where the words are.
