...
- **On web, the text is written into the page.** VoiceOver switches voice only for text written into the page, not for a name given as an `aria-label`, and Flutter's web engine names any node that has children with an `aria-label`. So a message whose words are their own buttons also carries its whole text as a text-only node, read before the words. A message without word buttons is already page text. VoiceOver on web still reads the word buttons themselves in the UI voice, whatever language they carry, so on web a message is heard in its own voice only through that whole-message node.
...
- **An example chip is one button named by its message.** The message's word buttons are left out of it, so its name is written into the page. Tapping the chip opens the toolbar, where the words are.
...
Whether the voice actually switches also depends on the listener's device. NVDA and JAWS switch only when that language's voice is installed and automatic language switching is on. A missing voice is not a defect in the app. Shared implementation: [`LanguageSemantics`](../../lib/pangea/common/widgets/language_semantics.dart).
