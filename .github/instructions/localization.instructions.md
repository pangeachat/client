...
Always run `flutter gen-l10n` after translating, then spot-check a few locales against English — especially non-Latin scripts and plural/placeholder keys — and commit the arb files plus `ai-translated-keys.json`. Script-variant locales (`zh` Simplified vs `zh_Hant` Traditional, `pt_BR` vs `pt_PT`) are pinned in the prompt via `LOCALE_NAME_OVERRIDES` in `translate_gemini.py` — a bare language name once yielded Traditional characters for `zh`. Add any new script-variant locale to that map; spot-checking those locales stays worthwhile as the backstop. Provenance lists in `ai-translated-keys.json` are written **sorted** per locale, so a future key adds one line; a one-time reorder-only diff there is expected, not corruption.
