...
|---|---|---|---|
| `code_tests` | **yes** | ~6-8.5m (the gate itself ~5s) | The gate the two jobs below report through, and the only name branch protection asks for — which is what lets the work under it be reshaped without a protection change. Red if either of them is red. |
...
| `code_test_shards` (4 jobs) | via `code_tests` | ~4m-7m each, in parallel | The Dart test suite. |
...
Four shards do not make it four times faster, and the gain moves between runs. Two runs of the same branch finished the required check in 5m39s and 8m29s. The difference was not queueing — every shard started within three seconds in both — but which shard drew the most expensive test file. In the slower run the shards took 225, 294, 311 and 428 seconds, ranking exactly by the heavy call-test file each received: the one with none finished first, the one with `active_call_test.dart` last. Round-robin balances the *number* of files, and files differ in cost by more than an order of magnitude. The shards' total also exceeds the unsplit run by about a sixth, since each repeats the fixed setup and starts its compiler cold.

That sets a floor no shard count can beat: the fixed setup, a cold compile, and then however long the single slowest file takes. This is why a fifth or sixth shard buys little — the longest file already has a runner to itself. Lowering the floor means making that file cheaper, by splitting `active_call_test.dart` or moving the call tests, not by adding runners.

Two more consequences worth knowing before editing. The organisation is on GitHub's Free plan, which allows 20 concurrent jobs across every repo it owns, and this change takes one pull request from four jobs to nine; when that pool is contended the shards queue and the gain erodes, so the timings above are a quiet-runner best case. And the shard list must keep matching what `flutter test` itself discovers — every file under `test/` whose name ends in `_test.dart`, symbolic links included, since Flutter resolves them. A shard that matches no files fails loudly rather than falling through to running everything, because the silent version of that mistake looks like a green build.
