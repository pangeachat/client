...
Removing the panel is not enough on its own. The asset is really two things: a confetti field that wants to be ambient, and one subject — the star carrying the two characters — that wants to be placed. Painted as a single background it cannot be both, and the subject ends up wherever the viewport happens to drop it, which is behind the content on a small screen. So the four settings surfaces draw the subject themselves, as [`StarCharacters`](../../lib/features/subscription/widgets/star_characters.dart) at the end of their scroll content, where nothing can cover it and nothing gets clipped to make room. The ambient field is then cropped to stop above the characters, so they are never painted twice. The onboarding free-trial step keeps the whole field and places nothing.
