...
- **Comparing results:** each run writes a result file. Results are comparable only on the same device and the same account, so the tool shows change over time, not a ranking of devices. A difference counts only when it's larger than the spread between repeated runs.
