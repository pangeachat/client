...
When a user-cluster button opens a panel, keyboard focus moves from that button to the panel's first control in Tab order, one discrete claim after the panel mounts, so the panel a learner just opened is where their next keypress lands. A panel opened any other way — a URL, the rail, a map pin — leaves focus where it was. The claim is armed by the cluster's open methods ([`UserClusterViewModel`](../../lib/routes/world/user_cluster_view_model.dart), through a one-shot [`PanelEntryIntent`](../../lib/features/navigation/panel_entry_intent.dart) that expires within a second so a press that opened nothing cannot move focus later) and taken by the right column's dispatcher ([`PanelEntryFocus`](../../lib/routes/world/right_panel/panel_entry_focus.dart)) when the panel mounts. Today that first control is the panel header's close or back button.
