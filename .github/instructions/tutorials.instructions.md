...
**Skipping is sequence-wide, and nearly every card carries the control.** Skip sits bottom-left with the sequence's title bottom-right (so back-to-back sequences read as different walkthroughs, not one that restarted), both above the progress bar. Skipping marks every not-yet-seen tutorial of the *running sequence* seen: the learner said no to the walkthrough, not to the card it happened to be showing — marking only the current tutorial let the rest partially re-offer the same walkthrough, which read as the skip not working. Armed cards carry it too — a tap there completes the step wherever it lands, the Skip label just says so honestly.

**A one-step run carries no Skip.** With nothing queued behind the card, skipping and finishing are the same act, and a tap anywhere already performs it — so the button is a second name for the gesture the learner is about to make, and reads as a promise that more cards are coming ([#8954](https://github.com/pangeachat/client/issues/8954)). The rule is about the **run**, not the tutorial: it is the filtered sequence's own step total that decides, so a longer walkthrough the seen-filter has narrowed to one remaining step drops the control as well.

Two further cards go without it: a branch, whose decline choice IS the sequence skip, and the **greeting** — it fronts a longer run the learner hasn't grasped yet, so "Skip" there is ambiguous between "skip this hello" (and get more cards anyway, which reads as broken) and "skip the whole walkthrough". The way out starts on the next card, where the run's progress bar shows what is being skipped.
