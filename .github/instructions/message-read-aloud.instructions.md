...
- **Gated by the same toggle.** Voice mode is not a second switch: `TtsController` gates every request on its `TtsUseCase`'s tool setting, so a voice-mode read still requires `audioIncomingMessages`. It changes only *how* a qualifying read is played, never *whether* one happens.
