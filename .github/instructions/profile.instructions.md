...
- **The published level is refreshed for the language being studied, and only that one.** It is republished at startup from this device's own analytics, so a level left wrong by an earlier bug corrects itself rather than waiting for the learner to next earn XP. It is set exactly, not raised — a level that has genuinely fallen, which happens when a learner blocks constructs, has to be able to go down, so this republish can lower a level as well as raise it.
- **Nothing is published from a device that has no analytics for the language yet.** A fresh install, a new device, or anything that resets the local store leaves it empty, and an empty store computes level 1. Publishing that would replace the learner's real level with 1 for everyone who reads their profile, so the republish is skipped until this device has actually pulled that language's analytics at least once.
- **Other languages are left alone, deliberately.** Only the active language's local data is current: it has just been synced from its analytics room, and blocked constructs are folded for it alone. Every other language's local total is frozen at whatever it was when that language was last studied here, so republishing from it would fight any other device the learner uses — each raising the shared value back to its own stale figure, in both directions, indefinitely. The active language is also the one other people see, since a profile's level is read from its target language. A learner whose older language shows a stale level can correct it by switching to that language. Repairing the rest would mean loading every language's analytics at startup, which is not worth the cost or the risk.
