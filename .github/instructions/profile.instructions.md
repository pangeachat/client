...
- **The published star total counts banked stars for one language.** A star is one orchestrator-awarded activity goal. The total counts saved sessions only, taking the learner's best single run of each activity, for the language that activity was played in — the same rule the learner's own star count follows, so the number a classmate sees is the number the learner sees. It is deliberately not a course total: what a course-scoped total means depends on that course's activities, and is resolved per course by the progression resolver ([quests.instructions.md](quests.instructions.md)).
- **Unlike a level, a star total may be re-derived and republished.** It is published when a session banks stars, and again at startup where the re-derived total comes out *higher* than the profile holds — so stars earned before this mirror existed appear without waiting for the learner to finish another activity. Two things make this safe where the level's republish was not. Stars are counted from the activity-session rooms themselves, which the client already holds as ordinary room state, rather than derived from a local analytics store that is empty on a fresh install and current only for the language being studied. And a star total only ever rises, so a device that is missing rooms computes a total that is too low and is ignored, where a level that has genuinely fallen (a learner blocking constructs) is indistinguishable from a device that is behind.
