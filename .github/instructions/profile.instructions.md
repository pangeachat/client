...
- **The published star total counts banked stars for one language.** A star is one orchestrator-awarded activity goal. The total counts saved sessions only, taking the learner's best single run of each activity, for the language that activity was played in — the same rule the learner's own star count follows, so a classmate normally sees the number the learner sees. It is deliberately not a course total: what a course-scoped total means depends on that course's activities, and is resolved per course by the progression resolver ([quests.instructions.md](quests.instructions.md)).
- **Unlike a level, a star total may be re-derived and republished.** It is published exactly by the path that banks stars at the end of a session, and re-derived at startup, where it is published only if it comes out *higher* than the profile already holds. The startup republish is what gives members who earned stars before this mirror existed a total, without waiting for them to finish another activity.
- **Why the startup republish is safe here and was not for levels.** Stars are counted from the activity-session rooms themselves, which the client holds as ordinary room state, rather than derived from a local analytics store that is empty on a fresh install and brought up to date only for the language being studied. Startup is still the wrong moment to *lower* a number, though, because a device part-way through its first sync is missing rooms and computes a total that is too low. Raising only makes that harmless: a device that is behind is ignored rather than believed.
- **A star total can genuinely fall, and corrects late when it does.** Leaving an activity-session room drops the stars earned in it from the count. The app offers no way to leave one, so this is rare rather than routine, but it is possible — and at startup it is indistinguishable from a device that has not finished syncing. A real fall is therefore published by the session-bank path, which runs while the app is up and the rooms are in hand, and until the learner next banks a session the mirror can read higher than their own count. That window is the accepted cost of not trusting a half-synced device.
