...
- Local `http://localhost:8090` (debug, against local or staging backends) — faithful proxy, fastest fix-verify loop. Set `ENABLE_SEMANTICS=true` in `client/.env` + clean-restart (see `run-flutter-web-local`). To reach a logged-in state without driving the canvas login form, open `http://localhost:8090/?devlogin=1` (debug-only, opt-in per load — signs into the test account via the `.env` creds; see `run-flutter-web-local` § Login).
