...
- **An app-side `onMapReady` camera nudge does NOT make the map render without a real gesture.** Tried (2026-06-21) forcing the initial tile load from the world map's `onMapReady` with a post-frame and a delayed programmatic `_controller.move` (longitude and zoom nudges). The move succeeds (`moved=true`, valid widget size) but yields **0 tile requests** — flutter_map's `TileLayer` (`didChangeDependencies` → `_loadAndPruneInVisibleBounds`) needs a *ready camera*, and the camera's `visibleBounds` still isn't ready until a real pointer gesture (pins don't load either). So the grey-map-until-interaction is a flutter_map-on-web gating, not app-fixable by a nudge — the **wake recipe stays necessary**. A real fix is a flutter_map/Flutter-web concern (minimal repro + upstream issue), not a quick app patch.
