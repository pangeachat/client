      ...
      builder: (context, snapshot) {
        ...
        return Padding(
          ...
          child: Semantics(
            ...
            container: true,
            // Every chat, session and open seat is one Tab stop, in the order
            // the list draws them, with the arrow keys inside; Tab lands on the
            // open chat (accessibility.instructions.md, "One Tab stop per
            // list"). The creation tiles, the what-now tile and load-more are
            // fixed in number and stay stops of their own.
            child: RovingFocusGroup(
              ids: [
                for (final chat in joinedChats) chat.id,
                for (final chunk in discoveredGroupChats) chunk.roomId,
                for (final session in joinedSessions) session.id,
                for (final entry in discoveredSessions)
                  for (final session in entry.value) session.chunk.roomId,
              ],
              selectedId: controller.widget.activeChat,
                ...
                itemBuilder: (context, i) {
                  ...
                  if (i < discoveredGroupChats.length) {
                    return UnjoinedChatListItem(
                      rovingId: discoveredGroupChats[i].roomId,
