...
void main() {
  ...
  // A trackpad pinch on the web reports the factor the gesture scaled by
  // (#8556). One zoom level is a doubling of scale, so the camera moves by the
  // factor's base-2 logarithm — the same amount a touch pinch of the same size
  // moves it — and lands inside the map's zoom range like every other zoom.
  group('WorldMapConstants.zoomAfterPinch (#8556)', () {
    const desktop = Size(1440, 900);
    final minZoom = WorldMapConstants.minZoomFor(desktop);

    test('a pinch that scales the world by 2 zooms in exactly one level', () {
      expect(WorldMapConstants.zoomAfterPinch(8, 2, minZoom), closeTo(9, 1e-9));
      expect(
        WorldMapConstants.zoomAfterPinch(8, 4, minZoom),
        closeTo(10, 1e-9),
      );
    });

    test('a pinch that halves the world zooms out exactly one level', () {
      expect(
        WorldMapConstants.zoomAfterPinch(8, 0.5, minZoom),
        closeTo(7, 1e-9),
      );
    });

    test('the many small factors of a real pinch accumulate to the same '
        'zoom as one big one', () {
      // The browser delivers a pinch as a stream of small scale factors, each
      // applied to the zoom the last one left — so ten factors of 2^0.1 must
      // land where a single factor of 2 does, or the gesture drifts.
      var zoom = 8.0;
      for (var i = 0; i < 10; i++) {
        zoom = WorldMapConstants.zoomAfterPinch(
          zoom,
          math.pow(2, 0.1) as double,
          minZoom,
        );
      }
      expect(zoom, closeTo(9, 1e-9));
    });

    test('a scale of 1 (no movement) leaves the camera alone', () {
      expect(WorldMapConstants.zoomAfterPinch(8, 1, minZoom), 8);
    });

    test('a hard pinch stops at the zoom ceiling and the viewport floor', () {
      expect(
        WorldMapConstants.zoomAfterPinch(17, 100, minZoom),
        WorldMapConstants.maxZoom,
      );
      expect(
        WorldMapConstants.zoomAfterPinch(minZoom + 0.5, 0.001, minZoom),
        minZoom,
      );
    });

    test('the floor it stops at is the viewport-derived one (#7813), not a '
        'fixed level', () {
      final phoneFloor = WorldMapConstants.minZoomFor(const Size(390, 844));
      expect(
        WorldMapConstants.zoomAfterPinch(4, 0.001, phoneFloor),
        phoneFloor,
      );
      expect(phoneFloor, lessThan(WorldMapConstants.fallbackMinZoom));
    });
  });
