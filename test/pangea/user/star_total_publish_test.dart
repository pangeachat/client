...
/// What reaches the server when a star total is published (#8438).
///
/// The published total is a high-water mark: it is raised, never lowered, and
/// a publish that changes nothing must not happen at all — the count is
/// recomputed on every sweep, so a writer that published each recount would PUT
/// the whole profile continuously. See profile.instructions.md.
void main() {
  ...
  test('raising the total publishes it under the language', () async {
    controller.userController.setPublicProfile(
      PublicProfileModel(analytics: AnalyticsProfileModel()),
      userId: client.userID,
    );

    await controller.userController.updateAnalyticsStars(
      languageCode: 'es',
      stars: 12,
    );
    ...
    final analytics =
        api.puts.single[AnalyticsConstants.analytics] as Map<String, dynamic>;
    expect(analytics['es'][AnalyticsConstants.stars], 12);
  });

  test('a recount that changes nothing does not publish', () async {
    controller.userController.setPublicProfile(
      PublicProfileModel(
        analytics: AnalyticsProfileModel(
          languageAnalytics: {
            'es': LanguageAnalyticsProfileEntry(3, 0, stars: 12),
          },
        ),
      ),
      userId: client.userID,
  ...
  test(
    'a lower count from a device that is behind is never published',
    () async {
      controller.userController.setPublicProfile(
        PublicProfileModel(
          analytics: AnalyticsProfileModel(
      ...
      expect(api.puts, isEmpty);
      expect(
        controller.userController.publicProfile!.analytics.starsByLanguage(
          'es',
        ),
        12,
      );
  ...
  test(
    'a total loaded for another account is never published to this one',
    () async {
      controller.userController.setPublicProfile(
        PublicProfileModel(analytics: AnalyticsProfileModel()),
        userId: _otherUserId,
