...
void main() {
  group('InvoiceSummary.fromJson', () {
    test('parses a full paid invoice (money in minor units)', () {
      ...
      expect(inv.created.toIso8601String(), "2026-07-01T12:00:00.000Z");
      expect(inv.created.isUtc, true);
    ...
    test('tolerates null number / urls / promo_code', () {
      ...
      expect(inv.status, InvoiceStatus.open);
    });
  ...
  group('InvoiceHistoryResponse.fromJson', () {
    test('parses a list of invoices', () {
      final res = InvoiceHistoryResponse.fromJson({
        "invoices": [
          {
            "id": "in_1",
            "created": "2026-07-01T12:00:00Z",
    ...
    test('no customers -> an empty invoices list', () {
      expect(
        InvoiceHistoryResponse.fromJson({"invoices": []}).invoices,
        isEmpty,
      );
      // Missing key also degrades to empty (defensive).
      expect(InvoiceHistoryResponse.fromJson({}).invoices, isEmpty);
    });
  });
}
