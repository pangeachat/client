...
void main() {
  ...
  OnboardingNavigationController controllerWithCachedCode({
    ...
    initialStep: ProfileSetupOnboardingStep(
      ...
      state: OnboardingStateController(
        ...
        trialInfoProvider: showTrialPage
            ? _TrialPageProvider()
            : MockTrialInfoProvider(),
      ),
      maxRemainingSteps: 5,
    ),
  );
  ...
  /// redeemed.
  Future<OnboardingNavigationController> joinedAs(
    UserType type, {
    bool showTrialPage = false,
  }) async {
    final state = controllerWithCachedCode(showTrialPage: showTrialPage);
  ...
  test('the trial page after the joined-course page still lands on the '
      'course (#8593)', () async {
    registerCourseSpace(courseLanguage: otherLanguage);
    final state = await joinedAs(UserType.student, showTrialPage: true);
    ...
    expect(await state.forward(), isA<SuccessNavigationResult>());
    expect(state.step, isA<JoinedCourseOnboardingStep>());
    // The trial page is still to come, so this is not the last step.
    expect(state.hasNextStep, isTrue);

    expect(await state.forward(), isA<SuccessNavigationResult>());
    expect(state.step, isA<FreeTrialOnboardingStep>());
    expect(state.hasNextStep, isFalse);
    // What the controller reads to decide where onboarding lands.
    expect(state.step.joinedCourseSpaceId, spaceId);
  });

  test('without a joined course onboarding lands on its plain destination', () {
    final step = getInitialOnboardingStep(client);
    expect(step.joinedCourseSpaceId, isNull);
  });
...
/// A user still inside their trial window, so the trial page follows the
/// joined-course page — the real shape for anyone who just signed up.
class _TrialPageProvider implements TrialInfoProvider {
  @override
