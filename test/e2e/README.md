...
The whole browser set, sequentially, stopping at the first failure. Nothing in
CI runs this folder -- `e2e-tests.yml` drives the separate `client/e2e`
Playwright suite, not this one -- so this loop is the entry point, run by hand,
and every browser scenario belongs in it, the two transcript scenarios included:
without them a green run says nothing about what either person reads back, and
the account could regress to one-half-per-device while the set stayed green.
They come last because they read speech back: the whole set already needs
`caller.wav` / `callee.wav` present (`fixtures/` holds them), and
`transcript_two_devices` additionally needs the learner's two wavs made above --
it REFUSES to start without them, so make them first or the set stops there:

    cd client
    for s in refresh_no_return scenarios refresh_midcall rejoin_ui grey_hover \
             transcript transcript_two_devices; do
...
- **The call membership state under-reports during that race.**
  ...
  timeline rather than the current state for that reason -- counting only the
  LIVE entries in each event, the way the state read does, because a reused
  fixture account also carries EXPIRED rows from past sessions and an expired
  row is not a device in the call. Unfiltered, the union over-reports: one
  device answering beside one stale row read as two joined, and every check
  gated on "both devices joined" passed on a call only one browser got into.
  Anything else asking "is the second device in the call" needs the same care.
...
- **A fixed sleep proves nothing about a drain of unbounded length.** The retry
  ...
  cannot see that half while it is still draining. (A PREVIOUS run's reuse is
  different: its RING is already in the room, written at placement, and the
  search finds it there whether or not that call's half has landed.) What
  settles the within-run case is that the harness watched both rings: every
  attempt's call key is remembered, and a winning key an earlier attempt already
  rang with is a reuse observed rather than inferred.
...
- **A whole call's transcript destroyed by a reused call key.** A redial two
  ...
  earlier ring or half already used, which is what turned this from silence into
  a failure. The search walks the room back to the event that IS the key -- a
  call key is the caller's own membership event id, and nothing can have been
  written under a key before it -- rather than reading a fixed window. A window
  answers "no earlier use in the last N events", which is a different sentence from the
