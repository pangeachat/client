...
/// `SubscriptionStatusV2Response` (status_v2_schema.py). JSON keys mirror the
...
void main() {
  ...
  group('SubscriptionStatusV2.fromJson', () {
    test('full active paid with planId', () {
      final status = SubscriptionStatusV2.fromJson(fullActivePaid());
      expect(status.accessLevel, "full");
      expect(status.entitlementSource, "cms");
      expect(status.manageEligible, true);

      final winning = status.winning!;
      expect(winning.type, "paid");
    ...
    test('none (no access, trial-eligible)', () {
      ...
      expect(status.accessLevel, "none");
    ...
    test('cancel_at_period_end set on the winning sub', () {
      ...
      final status = SubscriptionStatusV2.fromJson(json);
    ...
    test('comp (promotional, not cancelable)', () {
      ...
      expect(status.winning!.type, "comp");
    ...
    test('seat (group-managed, not cancelable)', () {
      ...
      expect(status.winning!.type, "seat");
      expect(status.entitlements.single.cancelable, false);
    });
    ...
    test('trial active', () {
      ...
      expect(status.winning!.type, "trial");
    ...
    test(
      ...
      () {
        final camel = SubscriptionStatusV2.fromJson({
          "access_level": "full",
          "entitlement_source": "cms",
          "winning": {
            "type": "paid",
        ...
        final snake = SubscriptionStatusV2.fromJson({
          "access_level": "full",
          "entitlement_source": "cms",
          "winning": {
            "type": "paid",
