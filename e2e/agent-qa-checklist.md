...
- [ ] Open a bare-code class link `/<code>` (seven `[a-z0-9]` chars, ≥1 digit) logged in, not a member — **Expect:** `LegacyRedirects` folds it into the add-course panel's private join leaf, which submits knock*with_code + join once (joining indicator, never the manual entry form) → user ends a joined member with no further interaction. *(joining-courses.instructions.md Route 1; [gated])\_
- [ ] Open the join link while logged OUT, then complete account creation — **Expect:** the class code persists on disk across login (`SpaceCodeRepo` via the `/` auth guard); after creation the guard replays the cached code into the join flow, which auto-joins. Do NOT clear browser storage mid-journey. _(joining-courses.instructions.md Route 1; [gated] [flaky])_
- [ ] Open a retired spelling `/join_with_link?classcode=XYZ` or `/join?classcode=XYZ` — **Expect:** NOT redirected (retired, not folded); the app boots at `/` with no join fired. The bare `/<code>` is the only inbound join shape. _(joining-courses.instructions.md Route 1; routing.instructions.md)_
...
- [ ] Open a bare-code class link in a MOBILE web browser, app not installed — **Expect:** no automatic store/scheme bounce (removed in #7830); Android shows the "Open in app" banner whose tap carries the current path via `intent://`; the Flutter web app loads and processes the code directly. _(joining-courses.instructions.md; [flaky] [staging-only])_
...
- **Analytics first-paint race.** The cluster (world user cluster, analytics bar, header avatar) and analytics surfaces must read inside a `StreamBuilder` subscribed during BUILD; `AnalyticsUpdateDispatcher` fires its first update once during init on a hot broadcast stream with no replay. A surface subscribing after init shows stale/zero until the next live update — "zero on first load" can be a real bug OR a too-early read.
